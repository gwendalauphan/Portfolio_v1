...
const Stars = (props) => {
  ...
  const [sphere] = useState(() =>
    random.inSphere(new Float32Array(6000), { radius: 6 })
  );
...
const StarsCanvas = () => {
  ...
  const [isDragging, setIsDragging] = useState(false); // État pour le drag

  useEffect(() => {
    const controls = controlsRef.current;

    if (!controls) return; // Ajout de cette vérification
